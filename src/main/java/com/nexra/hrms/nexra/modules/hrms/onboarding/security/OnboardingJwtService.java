...
public class OnboardingJwtService {
    ...
    public AuthenticatedOnboardingUser parseBearerToken(final String token) {
        ...
        if (products == null || !products.contains("HRMS")) {
            throw new SignatureException("User does not have HRMS product access.");
        }

        @SuppressWarnings("unchecked")
        List<String> roles = claims.get("roles", List.class);
        return new AuthenticatedOnboardingUser(
    ...
    private SecretKey signingKey() {
        ...
        if (bytes.length < 32) {
            throw new IllegalStateException("app.onboarding.security.jwt-secret must be at least 32 bytes");
        }
