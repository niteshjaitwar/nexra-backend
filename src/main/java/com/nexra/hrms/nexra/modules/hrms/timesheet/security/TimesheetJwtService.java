...
import com.nexra.hrms.nexra.modules.hrms.timesheet.config.TimesheetProperties;
...
public class TimesheetJwtService {
    private final TimesheetProperties properties;
    public AuthenticatedTimesheetUser parseBearerToken(final String token) {
        ...
        return new AuthenticatedTimesheetUser(UUID.fromString(claims.get("uid", String.class)), claims.getSubject(), claims.get("tenant", String.class), roles == null ? Set.of() : Set.copyOf(roles));
    }
    private SecretKey signingKey() {
        byte[] bytes = properties.getSecurity().getJwtSecret().getBytes(StandardCharsets.UTF_8);
        if (bytes.length < 32) throw new IllegalStateException("app.timesheet.security.jwt-secret must be at least 32 bytes");
